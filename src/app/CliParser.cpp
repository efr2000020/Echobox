...
const char* helpText() {
    return
        ...
        "                            If you also pass --cricket-filter on, this must be >= a\n"
        "                            derived floor (~16 ms at the shipped 384 kHz / hop-512)\n"
        "                            so the discard counter read cannot race an event still\n"
        "                            closing. The floor does not apply at the default\n"
        "                            --cricket-filter off; the shipped 20 ms clears it either\n"
        "                            way.\n"
        ...
        "                            When on, the detector's sweep-shape gate AND the\n"
        "                            recorder's post-hoc no-bat-like-event discard both\n"
        "                            engage. When off (default), both halves are disabled\n"
        "                            and the recorder behaves as if no filter existed.\n"
        "                            Default is off because the current gate rejects most\n"
        "                            real bats too: per-call recall measured 26 % with it\n"
        "                            on vs 99 % with it off on two field nights. Turn it\n"
        "                            on at a cricket-heavy site to trade recall for bytes.\n"
        "                            Interim — the gate is being redesigned.\n"
        "                            (default: off)\n"
        "\n"

...
These are the three §3 firmware-integrity checks. The plan's §4.1/§4.2
model cross-checks are not in this list: the Python shadow model they
validated is retired. See `tools/collection/README.md` for what that
does and does not buy us.

### 5a. The one thing the bench can no longer measure

The retired `verify_feature_parity` existed to measure x86 ↔ ARM
numerical drift. `echobox-replay` still runs x86 with `-march=native
-ffast-math` while the device runs ARM, so near the gate's hard
thresholds a per-event verdict can still flip between the two — and
nothing in the tree quantifies how often.

That does not block the bench rehearsal, because none of the three §3
checks above involve a re-implementation: they compare the device
against its own writers and against an externally injected signal. It
does mean a replay-derived **per-event** verdict carries an unquantified
error bar. Aggregate recall over thousands of events is far less
sensitive. See `VALIDATION_PROVENANCE.md` row 4b.
...
- [ ] The bench-rehearsal session directory is archived (rename it to
      ...
      session). The archive must contain `SESSION_HEADER.json` — it
      carries the firmware SHA and the full decision-relevant config,
      which is what lets a later auditor reproduce the run.
